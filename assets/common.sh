...
artifactory_current_version() {
  ...
  curl $1 | jq --arg v "$regex" '[.children[].uri | capture($v)]' | jq 'sort_by(.version | .[27:-13] | tonumber)' | jq '[.[length-1] | {version: .version}]'
...
artifactory_versions() {
  ...
  curl $1 | jq --arg v "$regex" '[.children[].uri | capture($v)]' | jq 'sort_by(.version | .[27:-13] | tonumber)' | jq '[.[] | {version: .version}]'
...
artifactory_files() {
  ...
  curl $1 | jq --arg v "$regex" '[.children[].uri | capture($v)]' | jq 'sort_by(.version | .[27:-13] | tonumber)' | jq '[.[] | {uri: .uri, version: .version}]'
